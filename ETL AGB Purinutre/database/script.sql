...
alter table if exists stage.stg_cliente
	add column if not exists nk_rota varchar(25),
	add column if not exists data_nascimento date,
	add column if not exists email varchar(255),
	add column if not exists cod_rota integer,
	add column if not exists rota varchar(255);

alter table if exists dim_cliente_compl
	add column if not exists data_nascimento date,
	add column if not exists email varchar(255),
	add column if not exists cod_rota integer,
	add column if not exists rota varchar(255);
	
alter table public.fato_movimento_venda_dia
	add column if not exists vlr_total_produto double precision default 0 not null;
...
create table if not exists dim_usuario_vistra
(
    sk_usuario_vistra serial not null,
    nk_usuario varchar(255) not null,
    usuario varchar(255),
    nome varchar(255),
    email varchar(255),
    tipousuario varchar(255),

    constraint dim_usuario_vistra_pkey primary key (sk_usuario_vistra),
    constraint dim_usuario_vistra_nk_cst unique (nk_usuario)
)with(oids=false);

drop table if exists stage.stg_mov_fin_nf_pag;
drop table if exists public.fato_mov_fin_nf_pag;

create table if not exists public.dim_tipo_pagamento
(
	sk_tipo_pagamento serial not null,
	nk_tipo_pagamento varchar(25) not null,
	tipo_pagamento varchar(255),
	dd_ativo smallint default 0 not null,
	
	constraint dim_tipo_pagamento_pkey primary key (sk_tipo_pagamento),
	constraint dim_tipo_pagamento_nk_cst unique (nk_tipo_pagamento),
	constraint dim_tipo_pag_ativo_cst check (dd_ativo in (0,1))
)with(oids=false);

create unlogged table if not exists stage.stg_mov_fin_nf_pag
(
	nk_empresa varchar(25),
	nk_cliente varchar(25),
	nk_tipo_pagamento varchar(25),
	nk_tempo_dia_pagamento date,
	dd_nota_fiscal varchar(25),
	dd_pag_a_vista smallint,
	numero_titulo integer,
	numero_parcela smallint,
...
create table if not exists public.fato_mov_fin_nf_pag
(
	sk_empresa integer not null,
	sk_cliente integer not null,
	sk_tipo_pagamento integer not null,
	sk_tempo_dia integer not null,
	dd_nota_fiscal varchar(25) not null,
	dd_pag_a_vista smallint,
	numero_titulo integer,
	numero_parcela smallint,
	vlr_desc double precision default 0 not null,
	vlr_juros double precision default 0 not null,
	vlr_multa double precision default 0 not null,
	vlr_pago double precision default 0 not null,
	
	constraint fato_mov_fin_nf_pag_empresa_fkey foreign key (sk_empresa) references dim_empresa(sk_empresa),
	constraint fato_mov_fin_nf_pag_cliente_fkey foreign key (sk_cliente) references dim_cliente(sk_cliente),
	constraint fato_mov_fin_nf_pag_tp_pag_fkey foreign key (sk_tipo_pagamento) references dim_tipo_pagamento(sk_tipo_pagamento),
	constraint fato_mov_fin_nf_pag_data_fkey foreign key (sk_tempo_dia) references dim_tempo_dia(sk_tempo_dia),
	constraint fato_mov_fin_nf_pag_pag_vista_cst check (dd_pag_a_vista in (0,1))
)with(oids=false);

comment on table public.fato_mov_fin_nf_pag is 'Rastreamento de pagamentos a vista e parcelados dos documentos de recebimento das notas fiscais';
comment on column public.fato_mov_fin_nf_pag.sk_tempo_dia is 'Data de pagamento a vista ou dos pagamentos das parcelas. Pode existir mais de um pagamento (com tipo diferente) pra mesma parcela/documento/nf/cliente/empresa.';

drop table if exists especifico.dim_colaborador_fluig;
create table if not exists especifico.dim_colaborador_fluig
(
    sk_colaborador_fluig serial not null,
    nk_colaborador_fluig varchar(25) not null,
    colaborador varchar(255) not null,
    codigo_fluig integer not null,
    codigo_pessoal_siagri integer,
    email varchar(255),
    perfil varchar(255),
    categoria varchar(255),

    constraint dim_colaborador_fluig_pkey primary key (sk_colaborador_fluig),
    constraint dim_colaborador_fluig_nk_cst unique (nk_colaborador_fluig)
)with(oids=false);

alter table if exists stage.stg_mov_fin_titulo_receber
    add column if not exists nk_vendedor varchar(25);

truncate table public.fato_mov_fin_titulo_receber;
alter table if exists public.fato_mov_fin_titulo_receber
    add column if not exists sk_vendedor integer not null,
    drop constraint if exists fato_mov_fin_tit_rec_vendedor_fkey,
    add constraint fato_mov_fin_tit_rec_vendedor_fkey foreign key (sk_vendedor) references dim_vendedor(sk_vendedor);
	
	
drop table if exists stage.stg_mov_fin_tit_reng_receber;
drop table if exists public.fato_mov_fin_tit_reng_receber;
create table if not exists stage.stg_mov_fin_tit_reng_receber
(
    nk_empresa varchar(25),
    nk_cliente varchar(25),
    nk_tempo_dia_reng date,
    nk_tempo_dia_venc date,
    nk_tipo_documento varchar(25),
    num_tit_original integer,
    numero_titulo integer,
    numero_parcela smallint,
    vlr_total_reng double precision,
    vlr_desconto double precision,
    vlr_juros double precision,
    vlr_parcela double precision
)with(oids=false);

create table if not exists public.fato_mov_fin_tit_reng_receber
(
    sk_empresa integer not null,
    sk_cliente integer not null,
    sk_tempo_dia_reng integer not null,
    sk_tempo_dia_venc integer not null,
    sk_tipo_documento integer not null,
    num_tit_original integer not null,
    numero_titulo integer not null,
    numero_parcela smallint not null,
    vlr_total_reng double precision default 0 not null,
    vlr_desconto double precision default 0 not null,
    vlr_juros double precision default 0 not null,
    vlr_parcela double precision default 0 not null,

    -- constraint fato_mov_fin_tit_reng_rec_pkey primary key (sk_empresa, sk_cliente, numero_titulo, numero_parcela), /* O mesmo documento possui parcelas repetidas */
    constraint fato_mov_fin_tit_reng_rec_empresa_fkey foreign key (sk_empresa) references dim_empresa (sk_empresa),
    constraint fato_mov_fin_tit_reng_rec_cliente_fkey foreign key (sk_cliente) references dim_cliente (sk_cliente),
    constraint fato_mov_fin_tit_reng_rec_data_reng_fkey foreign key (sk_tempo_dia_reng) references dim_tempo_dia (sk_tempo_dia),
    constraint fato_mov_fin_tit_reng_rec_data_venc_fkey foreign key (sk_tempo_dia_venc) references dim_tempo_dia (sk_tempo_dia),
    constraint fato_mov_fin_tit_reng_rec_tipo_doc_fkey foreign key (sk_tipo_documento) references dim_tipo_documento (sk_tipo_documento)
)with(oids=false);

comment on column fato_mov_fin_tit_reng_receber.vlr_total_reng is 'Valor total da renegociacao. Metrica semi-aditiva com base no documento de renegociacao, empresa e cliente.';
comment on table fato_mov_fin_tit_reng_receber is 'Fato semi-aditiva para o valor total renegociado.';

alter table if exists dim_produto
    add column fator_conversao double precision default 0;
	
alter table if exists dim_produto
    add column if not exists qtd_embalagem_primaria double precision default 0,
    add column if not exists qtd_embalagem_secundaria double precision default 0;
	
/**#################################################
 * Author: Flavio Alexsandro Silva
 * Date:   09/02/2022
 * Comment:
 *  Fatos para armazenamento dos dados de titulos a pagar
 *   e tambem do movimento dos pagamentos das parcelas
 *   destes titulos
 **###############################################**/

alter table dim_tipo_pagamento
    add column if not exists abrev_meio_pagamento varchar(2) default ' ' not null,
    add column if not exists meio_pagamento varchar(255);

alter table dim_tipo_pagamento
    alter column abrev_meio_pagamento drop default;

drop table if exists stage.stg_mov_fin_titulo_pagar;
drop table if exists public.fato_mov_fin_titulo_pagar;
drop table if exists public.fato_mov_fin_titulo_pago;
drop table if exists stage.stg_mov_fin_titulo_pago;

create unlogged table if not exists stage.stg_mov_fin_titulo_pagar
(
    nk_empresa varchar(25),
    nk_fornecedor varchar(25),
    nk_tipo_documento varchar(25),
    ...
    dd_doc_pagar varchar(100),
    dd_num_parcela integer,
    vlr_total_pagar double precision,
    vlr_desc_pont double precision,
    vlr_desc_antecp double precision,
    vlr_multa double precision,
    vlr_juros double precision,
    vlr_pago double precision,
    vlr_parcela double precision,
...
create table if not exists public.fato_mov_fin_titulo_pagar
(
    sk_empresa integer not null,
    sk_fornecedor integer not null,
    sk_tipo_documento integer not null,
    sk_tempo_dia_mov integer not null,
    sk_tempo_dia_venc integer not null,
    sk_tempo_dia_baixa integer not null,

    dd_doc_pagar varchar(100) not null,
    dd_num_parcela integer not null default 1,
    vlr_total_pagar double precision default 0,
    vlr_desc_pont double precision default 0,
    vlr_desc_antecp double precision default 0,
    vlr_multa double precision default 0,
    vlr_juros double precision default 0,
    vlr_pago double precision default 0,
    vlr_parcela double precision default 0,
    vlr_saldo double precision default 0,

    constraint fato_mov_fin_tit_pagar_empresa_fkey foreign key (sk_empresa) references dim_empresa (sk_empresa),
    constraint fato_mov_fin_tit_pagar_fornecedor_fkey foreign key (sk_fornecedor) references dim_fornecedor (sk_fornecedor),
    constraint fato_mov_fin_tit_pagar_tipo_doc_fkey foreign key (sk_tipo_documento) references dim_tipo_documento (sk_tipo_documento),
    constraint fato_mov_fin_tit_pagar_data_mov_fkey foreign key (sk_tempo_dia_mov) references dim_tempo_dia (sk_tempo_dia),
    constraint fato_mov_fin_tit_pagar_data_venc_fkey foreign key (sk_tempo_dia_venc) references dim_tempo_dia (sk_tempo_dia),
    constraint fato_mov_fin_tit_pagar_data_baixa_fkey foreign key (sk_tempo_dia_baixa) references dim_tempo_dia (sk_tempo_dia)
)with(oids=false);

create unlogged table if not exists stage.stg_mov_fin_titulo_pago
(
    nk_empresa varchar(25),
    nk_fornecedor varchar(25),
    nk_tipo_pagamento varchar(25),
...
create table if not exists fato_mov_fin_titulo_pago
(
    sk_empresa integer not null,
    sk_fornecedor integer not null,
    sk_tipo_pagamento integer not null,
    ...
    dd_doc_pagar varchar(100),
    dd_num_parcela integer default 1,
    vlr_desc double precision default 0,
    vlr_juros double precision default 0,
    vlr_multa double precision default 0,
    vlr_pago double precision default 0,

    constraint fato_mov_fin_tit_pago_empresa_fkey foreign key (sk_empresa) references dim_empresa (sk_empresa),
    constraint fato_mov_fin_tit_pago_fornecedor_fkey foreign key (sk_fornecedor) references dim_fornecedor (sk_fornecedor),
    constraint fato_mov_fin_tit_pago_tip_pag_fkey foreign key (sk_tipo_pagamento) references dim_tipo_pagamento (sk_tipo_pagamento),
    constraint fato_mov_fin_tit_pago_tempo_dia_fkey foreign key (sk_tempo_dia) references dim_tempo_dia (sk_tempo_dia)
)with(oids=false);

create table if not exists public.dim_centro_custo (
	sk_centro_custo serial4 NOT NULL,
	nk_centro_custo varchar(255) NOT NULL,
	nome_centro_custo varchar(255) NULL,
	situacao int4 NOT NULL,
	CONSTRAINT dim_centro_custo_pkey PRIMARY KEY (sk_centro_custo)
);

comment on table public.fato_mov_fin_titulo_pagar is 'Fato dos titulos a pagar';
comment on column public.fato_mov_fin_titulo_pagar.vlr_total_pagar is 'Coluna semi-aditiva, que representa o valor total do titulo. Eh unica com base nos dados empresa, fornecedor e doc_pagar';
comment on column public.fato_mov_fin_titulo_pagar.sk_tempo_dia_mov is 'Coluna que representa o dia mais proximo ou provavel da geracao ou recepcao do titulo, pois nao ha a informacao de data de geracao do titulo.';
comment on table public.fato_mov_fin_titulo_pago is 'Fato dos pagamentos realizados para os titulos a pagar. Todos os movimentos, nao estornados.';

alter table if exists stage.stg_mov_fin_titulo_receber
    rename column numero_titulo to dd_titulo;
alter table if exists stage.stg_mov_fin_titulo_receber
    rename column numero_parcela to dd_parcela;
alter table if exists stage.stg_mov_fin_nf_pag
    rename column numero_titulo to dd_titulo;
alter table if exists stage.stg_mov_fin_nf_pag
    rename column numero_parcela to dd_parcela;

alter table if exists fato_mov_fin_titulo_receber
    rename column numero_titulo to dd_titulo;
alter table if exists fato_mov_fin_titulo_receber
    rename column numero_parcela to dd_parcela;
alter table if exists fato_mov_fin_nf_pag
    rename column numero_titulo to dd_titulo;
alter table if exists fato_mov_fin_nf_pag
    rename column numero_parcela to dd_parcela;

alter table if exists stage.stg_mov_fin_titulo_receber
    add column if not exists dd_reng smallint default 0;

alter table if exists public.fato_mov_fin_titulo_receber
    add column if not exists dd_reng smallint default 0 not null;
	
drop table if exists stage.stg_mov_fin_tit_reng_receber restrict;
drop table if exists fato_mov_fin_tit_reng_receber restrict;

drop table if exists stage.fluxo_caixa_realizado_dia;

create unlogged table if not exists stage.fluxo_caixa_realizado_dia
(
    dia date,
    cod_filial integer,
    cod_conta_fluxo_caixa integer,
    valor_entrada float8,
    valor_saida float8
)with(oids=false);


create table if not exists public.dim_tipo_cobranca (
       sk_tipo_cobranca serial4 NOT NULL,
       nk_tipo_cobranca varchar(30) NULL,
       estado int,
       lista_fluxo int,
       cobranca varchar(30) NOT NULL,
       CONSTRAINT dim_tipo_cobranca_pkey PRIMARY KEY (sk_tipo_cobranca)
);
